...
Скрипт проходится по словарю hosts.json и для каждого элемента создает ip запись в ipam netbox, владельцем ресурса по умолчанию выставляется пользователь запустивший action, из ldap подтягивается почтовый адрес пользователя, необходимый в netbox в поле owner
